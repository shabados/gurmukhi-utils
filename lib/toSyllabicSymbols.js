...
const multipleDeeraghSymbols = new RegExp( /a+/, 'g' )
const twoUnitSequence = new RegExp( /ba/, 'g' )
const oneUnitSymbol = new RegExp( /b/, 'g' )
...
const toSyllabicSymbols = ( text ) => toUnicode( text ) // Sanitize input
  ...
  .replace( multipleDeeraghSymbols, 'a' ) // Only one long sound is enough to modify base char
  .replace( twoUnitSequence, '2' ) // Combine a base char and long sound into a 2 unit syllable
  .replace( oneUnitSymbol, '1' ) // Convert any remaining base chars to a 1 unit syllable
