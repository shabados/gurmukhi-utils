...
const extraARules = [
  ...
  letter => letter.toLowerCase().indexOf( 'aeiou ooaiee' ) === -1,
  ...
  ( _, nextLetter ) => nextLetter.toLowerCase().indexOf( 'aeouyw' ) === -1,
  // Next letter is not in long string of stuff
  ( _, nextLetter ) => nextLetter.indexOf( 'I@ HRªÅÆÇÍÏÒÓÔØÚåæçüŒœ:[]()' ) === -1,
  ( _, nextLetter, nextNextLetter ) => !console.log( _, nextLetter, nextNextLetter ),
  // Next letter is not i, third letter is not empty and is a space
  //! This line causing issues?
  //! Removing first condition makes it a bit better?
...
const uReplacements = Object.entries( {
...
const iReplacements = Object.entries( {
